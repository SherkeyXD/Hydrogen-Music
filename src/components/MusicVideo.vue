...
const loginHandle = async (data) => {
    ...
    localStorage.setItem(
        "Sessdata",
        data.url.match(/SESSDATA=(\S*)&bili_jct/)[1],
    );
...
const search = async () => {
    if (videoUrl.value == "" || !videoUrl.value) {
        ...
        if (bv) {
            ...
            const videoInfo = await windowApi.getRequestData({
                url: "http://api.bilibili.com/x/web-interface/view",
                option: { headers: headers, params: { bvid: bv } },
            });
            if (videoInfo.code == 0) {
                ...
                const videoData = await windowApi.getRequestData({
                    ...
                    option: {
                        ...
                        params: {
                            bvid: bv,
                            cid: currentVideoInfo.value.cid,
                            fourk: 1,
                            fnval: 80,
                        },
                    },
                });
                currentVideoInfo.value.quality =
...
const checkUrl = () => {
    if (videoUrl.value.indexOf("video/BV") != -1)
        return videoUrl.value.split("/")[4];
    else if (videoUrl.value.indexOf("BV") != -1 && videoUrl.value[0] == "B")
...
watch(
    ...
    () => {
        ...
        if (vInterval > mInterval)
            videoTiming.value = [
                videoTiming.value[0],
                (videoTiming.value[1] -= vInterval - mInterval),
            ];
    },
);
const addTiming = () => {
    ...
    if (timingList.value == null) {
        timingList.value = [
            { start: start, end: end, videoTiming: videoTiming.value[0] },
        ];
    } else {
        for (let i = 0; i < timingList.value.length; i++) {
            if (
                !(
                    (start < timingList.value[i].start &&
                        end < timingList.value[i].start) ||
                    (start > timingList.value[i].end &&
                        end > timingList.value[i].end)
                )
            ) {
        ...
        timingList.value.push({
            ...
            videoTiming: videoTiming.value[0],
        });
    }
...
const addConfirm = () => {
    ...
    if (addMusicVideo.value.isSave)
        dialogOpen(
            "确认添加",
            "本歌曲在本地已保存有视频记录，继续添加将覆盖旧数据，您确定吗？",
            addVideo,
        );
    else addVideo(true);
};
const addVideo = async (flag) => {
    ...
    let urlIndex =
        selectedInfo.value.qn -
        (currentVideoInfo.value.quality.length -
            currentVideoInfo.value.video.length / 2);
...
const deleteConfirm = () => {
    if (addMusicVideo.value.isSave) {
        dialogOpen(
            "确认删除",
            "本歌曲在本地已保存有视频记录，您确定要删除吗？若需要删除本地文件请前往设置。",
            deleteVideo,
        );
    } else {
        setDefault();
...
const loadData = () => {
    if (addMusicVideo.value != null) {
        windowApi
            .musicVideoIsExists({ id: addMusicVideo.value.id, method: "get" })
            .then((result) => {
                if (result) {
        ...
        <div class="set-video-container">
            ...
            <div class="set-video-title">
                ...
                <span class="title-en"
                    ...
                    <span style="color: pink">{{
                        addMusicVideo ? addMusicVideo.name : "Muisc"
                    }}</span></span
                >
            </div>
            <div class="set-video-info">
                <div class="bili-account">
                    <div class="account-info">
                        ...
                        <div class="info" v-if="userStore.biliUser">
                            <img
                                :src="userStore.biliUser.face + '@100w'"
                                alt=""
                            />
                            <div class="my-info">
                                <span class="username">{{
                                    userStore.biliUser.uname
                                }}</span>
                                <span class="vip"
                                    >大会员：{{
                                        userStore.biliUser.vipStatus
                                            ? "已激活"
                                            : "未激活"
                                    }}</span
                                >
                            ...
                            <div
                                class="info-back"
                                :class="{
                                    'info-back2': !userStore.biliUser.vipStatus,
                                }"
                            >
                                BILIBILI VIP
                            </div>
                        </div>
                        <div class="no-info" v-if="!userStore.biliUser">
                            NONE
                        </div>
                    </div>
                    <div class="account-button" @click="loginOrLogout()">
                        {{ userStore.biliUser ? "退出" : "登录" }}
                    </div>
                </div>
                <div class="video-url">
                    <input
                ...
                <div class="video-add">
                    <div class="video-info">
                        <div class="video-quality" v-if="selectedInfo.quality">
                            ...
                            <span class="quality">{{
                                selectedInfo.quality.substring(3)
                            }}</span>
                        </div>
                        ...
                        <Transition name="fade">
                            <div class="video" v-if="currentVideoInfo">
                                ...
                                <div
                                    class="more-video"
                                    v-if="currentVideoInfo.pages.length != 1"
                                >
                                    <div
                                        class="video-p"
                                        :class="{
                                            'opt-selected':
                                                selectedInfo.part == item.cid,
                                        }"
                                        v-for="(
                                            item, index
                                        ) in currentVideoInfo.pages"
                                    >
                                        <span
                                            class="p-name"
                                            :title="item.part"
                                            @click="selectPart(item.cid)"
                                            >{{ item.part }}</span
                                        >
                                    </div>
                                </div>
                                <div
                                    class="video-all-quality"
                                    v-if="currentVideoInfo.quality"
                                >
                                    <div
                                        class="quality-list"
                                        :class="{
                                            'opt-selected':
                                                selectedInfo.qn == index,
                                        }"
                                        v-for="(
                                            item, index
                                        ) in currentVideoInfo.quality"
                                    >
                                        <span
                                            class="quality-opt"
                                            :title="item"
                                            @click="selectQuality(item, index)"
                                            >{{ item }}</span
                                        >
                                    </div>
                                </div>
                        ...
                        <Transition name="fade">
                            <div class="no-video" v-if="!currentVideoInfo">
                                NONE
                            </div>
                        </Transition>
                    </div>
                    <div class="video-other">
                        ...
                        <div class="video-delete" @click="deleteConfirm()">
                            删除
                        </div>
                    </div>
                </div>
                <Transition name="fade">
                    ...
                    >
                        ...
                        <div class="timing-axis">
                            ...
                            <span class="time-label">{{
                                songTime2(currentVideoInfo.duration)
                            }}</span>
                        </div>
                    </div>
                </Transition>
                <Transition name="fade">
                    <div
                        class="add-timing"
                        v-if="selectedInfo.part && selectedInfo.quality"
                        @click="addTiming()"
                    >
            ...
            <Transition name="fade">
                <div
                    ...
                    v-if="timingList && currentVideoInfo"
                >
                        ...
                        <TransitionGroup name="list" tag="div">
                            ...
                            >
                                <span
                                    ...
                                    "
                                    >M: {{ formatTime(item.start) }} -
                                    {{ formatTime(item.end) }}<br />V:
                                    {{ formatTime(item.videoTiming) }} -
                                    {{
                                        formatTime(
                                            item.videoTiming +
                                                item.end -
