...
const play = (song, index) => {
    if (!song.playable) {
        noticeOpen(
            `当前歌曲无法播放${!!song.reason ? ", " + song.reason : ""}`,
            2,
        );
        return;
    }
...
const openMenu = (e, item) => {
    ...
    if (
        otherStore.selectedPlaylist &&
        otherStore.selectedPlaylist.creator &&
        otherStore.selectedPlaylist.creator.userId == userStore.user.userId
    )
            ...
            >
                ...
                <div class="item-other">
                    ...
                    <span class="item-time">{{
                        songTime(item.dt || item.duration)
                    }}</span>
                </div>
            </div>
        </RecycleScroller>
