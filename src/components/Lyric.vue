...
import { changeProgress, musicVideoCheck, songTime } from "../utils/player.mjs";
...
const lyricTypeCheck = (arr, tarr, rarr) => {
    if (arr && lyricType.value.indexOf("noOriginal") != -1)
        lyricType.value.splice(lyricType.value.indexOf("noOriginal"), 1);
    else if (!arr && lyricType.value.indexOf("noOriginal") == -1)
        lyricType.value.push("noOriginal");
    if (tarr && lyricType.value.indexOf("noTrans") != -1)
        lyricType.value.splice(lyricType.value.indexOf("noTrans"), 1);
    else if (!tarr && lyricType.value.indexOf("noTrans") == -1)
        lyricType.value.push("noTrans");
    if (rarr && lyricType.value.indexOf("noRoma") != -1)
        lyricType.value.splice(lyricType.value.indexOf("noRoma"), 1);
    else if (!rarr && lyricType.value.indexOf("noRoma") == -1)
        lyricType.value.push("noRoma");
};
...
const lyricHandle = (arr, tarr, rarr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if (obj.lyric.indexOf("纯音乐") != -1 || obj.time > 4500) {
            lyricArr = [
                { lyric: "纯音乐，请欣赏", time: 0 },
                {
                    lyric: "",
                    time: Math.trunc(
                        songList.value[currentIndex.value].dt / 1000,
                    ),
                },
            ];
            return lyricArr;
        ...
        if (
            tarr &&
            obj.lyric.indexOf("作词") == -1 &&
            obj.lyric.indexOf("作曲") == -1
        ) {
            for (let j = 0; j < tarr.length; j++) {
                if (tarr[j] == "") continue;
                if (
                    tarr[j].indexOf(
                        lyctime[0].substring(0, lyctime[0].length - 1),
                    ) != -1
                ) {
                    obj.tlyric =
                        tarr[j].split("]")[1].trim() === ""
        ...
        obj.time = lyctime
            ? formatLyricTime(lyctime[0].slice(1, lyctime[0].length - 1))
            : 0;
...
const getLyric = computed(() => {
    if (
        (!widgetState.value && lyric.value) ||
        (lyric.value && !lyricsObjArr.value)
    ) {
        if (lyric.value.lrc.lyric.indexOf("[") != -1) {
            lyricsObjArr.value = lyricHandle(
                lyric.value.lrc.lyric.split(regNewLine),
                lyric.value.tlyric && lyric.value.tlyric.lyric
                    ? lyric.value.tlyric.lyric.split(regNewLine)
                    : null,
                lyric.value.romalrc && lyric.value.romalrc.lyric
                    ? lyric.value.romalrc.lyric.split(regNewLine)
                    : null,
            );
            setDefaultStyle();
            ...
            lyricsObjArr.value.push({
                lyric: "",
                time: Math.trunc(songList.value[currentIndex.value].dt / 1000),
            });
...
const clearLycAnimation = (flag) => {
    ...
    for (let i = 0; i < lyricEle.value.length; i++) {
        ...
        if (lyricBlur.value)
            lyricEle.value[i].firstChild.style.setProperty(
                "filter",
                "blur(0px)",
            );
    }
...
const setMaxHeight = (change) => {
    ...
    size =
        (parseInt(
            lyricType.value.indexOf("noOriginal") == -1 &&
                lyricType.value.indexOf("original") != -1
                ? lyricSize.value
                : 0,
        ) +
            parseInt(
                lyricType.value.indexOf("noTrans") == -1 &&
                    lyricType.value.indexOf("trans") != -1
                    ? tlyricSize.value
                    : 0,
            ) +
            parseInt(
                lyricType.value.indexOf("noRoma") == -1 &&
                    lyricType.value.indexOf("roma") != -1
                    ? rlyricSize.value
                    : 0,
            )) *
            1.5 +
...
const setLyricActive = () => {
    ...
    lyricInterval.value = setInterval(() => {
        ...
        lycCurrentIndex.value = lyricsObjArr.value.findIndex((item, index) => {
            if (index != length) {
                return (
                    (currentSeek + 0.2) * 1000 <
                    lyricsObjArr.value[index + 1].time * 1000
                );
            }
            if (index == length) {
        ...
        if (lastIndex != lycCurrentIndex.value) {
            ...
            if (lyricShow.value && isLyricDelay.value && lyricEle.value) {
                if (lyricBlur.value)
                    for (
                        let i = 0, j = lycCurrentIndex.value * 0.4;
                        i < lycCurrentIndex.value;
                        i++
                    ) {
                        lyricEle.value[i].firstChild.style.setProperty(
                            "filter",
                ...
                for (
                    let i = lycCurrentIndex.value, j = 0, k = 0;
                    i < lyricEle.value.length;
                    i++
                ) {
            ...
            if (lycCurrentIndex.value != length)
                interTime =
                    lyricsObjArr.value[lycCurrentIndex.value + 1].time -
                    currentSeek;
        ...
        if (
            interludeAnimation.value &&
            lyricsObjArr.value[lycCurrentIndex.value + 1].time - currentSeek < 1
        ) {
            interludeAnimation.value = false;
            clearTimeout(interludeOutTimer);
            interludeOutTimer = setTimeout(() => {
            ...
            interludeRemainingTime.value = Math.trunc(
                lyricsObjArr.value[lycCurrentIndex.value + 1].time -
                    currentSeek -
                    1,
            );
...
watch(
    () => [lyricSize.value, lyricType.value],
    () => {
...
onMounted(() => {
    lyricScroll.value.addEventListener("wheel", (e) => {
        ...
        heightVal.value +=
            e.wheelDeltaY < 0 ? e.wheelDeltaY + 76 : e.wheelDeltaY - 76;

        if (heightVal.value < minHeightVal.value)
            heightVal.value = minHeightVal.value;
        if (heightVal.value > maxHeightVal.value)
            heightVal.value = maxHeightVal.value;
...
<template>
    <div class="lyric-container">
        <Transition name="fade">
            ...
            >
                ...
                >
                    <div
                        class="line"
                        @click="changeProgressLyc(item.time, index)"
                        :class="{
                            'line-highlight': index == lycCurrentIndex,
                            'lyric-inactive': !isLyricActive || item.active,
                        }"
                    >
                        <span
                            class="roma"
                            :style="{ 'font-size': rlyricSize + 'px' }"
                            v-if="
                                item.rlyric && lyricType.indexOf('roma') != -1
                            "
                            >{{ item.rlyric }}</span
                        >
                        <span
                            ...
                            v-if="lyricType.indexOf('original') != -1"
                            >{{ item.lyric }}</span
                        >
                        <span
                            class="trans"
                            :style="{ 'font-size': tlyricSize + 'px' }"
                            v-if="
                                item.tlyric && lyricType.indexOf('trans') != -1
                            "
                            >{{ item.tlyric }}</span
                        >
                        <div
                            class="hilight"
                            :class="{
                                'hilight-active': index == lycCurrentIndex,
                            }"
                            :style="{
                                backgroundColor: videoIsPlaying
                                    ? 'rgba(0, 0, 0, 0.8)'
                                    : 'black',
                            }"
                        ></div>
                    ...
                    <div
                        v-if="lycCurrentIndex != -1 && interludeIndex == index"
                        class="music-interlude"
                        :class="{ 'music-interlude-in': interludeAnimation }"
                    >
                            ...
                            <div class="interlude-title">
                                ...
                                <div class="title-style">
                                    ...
                                    >
                                        ...
                                        <g
                                            opacity="1"
                                            transform="translate(0 0)  rotate(0 24.5 25)"
                                        >
        ...
        <Transition name="fade">
            <div
                v-show="!lyricsObjArr || lyricType.indexOf('original') == -1"
                class="lyric-nodata"
            >
                <div class="line1"></div>
        ...
        <span class="song-quality" v-if="songList[currentIndex].type == 'local'"
            >{{ songList[currentIndex].sampleRate }}KHz/{{
                songList[currentIndex].bitsPerSample
            }}Bits/{{ songList[currentIndex].bitrate }}Kpbs</span
        >
        <span class="song-quality" v-if="songList[currentIndex].level"
            >{{ songList[currentIndex].level.sr / 1000 }}KHz/{{
                Math.round(songList[currentIndex].level.br / 1000)
            }}Kpbs/{{ songList[currentIndex].quality.toUpperCase() }}</span
        >
...
.lyric-container {
            ...
            .music-interlude {
                ...
                .interlude-left {
                    .diamond {
                        ...
                        animation: diamond-rotate 1.6s 0.6s
                            cubic-bezier(0.3, 0, 0.12, 1) infinite;
                        position: relative;
    ...
    .lyric-nodata {
        ...
        .line2 {
            ...
            animation: nodata-open1 0.8s 0.5s
                cubic-bezier(0.32, 0.81, 0.56, 0.98) forwards;
