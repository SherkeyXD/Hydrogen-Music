...
import { songTime2, changeProgress } from "@/utils/player.mjs";
...
<template>
    <div class="title-container">
        <Transition name="fade">
            <div class="title-logo" @click="backHome()" v-show="playerShow">
                Hydrogen
            </div>
        </Transition>
        <div
            ...
            v-if="musicVideo && songList"
            @click="playerShow = true"
        >
            <div
                class="player-content"
                :class="{ 'player-content-in': videoIsPlaying && !playerShow }"
            >
                <div class="cover">
                    <img
                        v-if="songList[currentIndex].type != 'local'"
                        :src="
                            songList[currentIndex].al.picUrl + '?param=100y100'
                        "
                        alt=""
                    />
                    <img
...
.title-container {
    ...
    .title-player {
        ...
        left: 0;
        z-index: 999;
        transition: 0.5s cubic-bezier(0.3, 0.79, 0.55, 0.99);
        visibility: hidden;
        overflow: hidden;
        ...
        .player-content {
            ...
            transition: 0.2s 1s cubic-bezier(0.06, 0.52, 0.29, 1);
            opacity: 0;
            .cover {
                margin-right: 8px;
                img {
        ...
        .player-content-in {
            ...
            transition: 0.8s 1.2s cubic-bezier(0.06, 0.52, 0.29, 1);
        }
    }
    .title-player-in {
        width: 32vh;
        visibility: visible;
        transition: 0.4s 0.8s cubic-bezier(0.06, 0.52, 0.29, 1);
    }
}
.fade-enter-active,
.fade-leave-active {
