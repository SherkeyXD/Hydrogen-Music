...
import { subPlaylist } from "../api/playlist.mjs";
import { subAlbum } from "../api/album.mjs";
import { subArtist } from "../api/artist.mjs";
...
const createTime = computed(() => {
    return dayjs(
        libraryInfo.value.createTime || libraryInfo.value.publishTime,
    ).format("YYYY-MM-DD");
});
...
const subHandle = (id) => {
    ...
    if (isAlbum.value) {
        ...
        noCacheTimer = setTimeout(() => {
            needTimestamp.value.splice(
                needTimestamp.value.indexOf("/album/sublist"),
                1,
            );
    ...
    if (isSinger.value) {
        ...
        noCacheTimer = setTimeout(() => {
            needTimestamp.value.splice(
                needTimestamp.value.indexOf("/artist/sublist"),
                1,
            );
            clearTimeout(noCacheTimer);
        }, 130000);
    ...
    if (!libraryInfo.value.followed) {
        ...
        if (listType1.value == type1 && listType2.value == type2)
            libraryList.value.unshift(libraryInfo.value);
        return;
    }
};
    ...
    <div class="library-detail" v-if="libraryInfo">
        ...
        <div class="library-introduce">
            <div class="introduce">
                <div
                    class="introduce-img"
                    :class="{ 'introduce-img-circle': isSinger }"
                >
                    <img
                        :src="
                            (libraryInfo.coverImgUrl ||
                                libraryInfo.blurPicUrl ||
                                libraryInfo.img1v1Url) + '?param=300y300'
                        "
                        alt=""
                    />
                </div>
                <div class="introduce-info">
                    ...
                    <div class="info-other">
                        <div class="introduce-author">
                            <span class="author" v-if="libraryInfo.creator">{{
                                libraryInfo.creator.nickname
                            }}</span>
                            <span
                                class="author"
                                @click="checkArtist(artist.id)"
                                v-for="(artist, index) in libraryInfo.artists"
                                >{{ artist.name
                                }}{{
                                    index == libraryInfo.artists.length - 1
                                        ? ""
                                        : "/"
                                }}</span
                            >
                            <span class="author" v-if="libraryInfo.trans"
                                >{{ libraryInfo.trans }}&nbsp;&nbsp;</span
                            >
                            <span
                                class="author"
                                v-for="(alia, index) in libraryInfo.alias"
                                >{{ alia
                                }}{{
                                    index == libraryInfo.alias.length - 1
                                        ? ""
                                        : " · "
                                }}</span
                            >
                        </div>
                        <span class="introduce-num" v-if="!isSinger"
                            >共{{
                                libraryInfo.trackCount || libraryInfo.size
                            }}首 - {{ totalTime }}分钟</span
                        >
                        <span class="introduce-num" v-if="isSinger"
                            >{{ libraryInfo.musicSize }}首歌 ·
                            {{ libraryInfo.albumSize }}张专辑 ·
                            {{ libraryInfo.mvSize }}个MV</span
                        >
                        <div class="library-operation" v-if="isLogin()">
                            <div
                                class="operation-collection"
                                v-show="
                                    (playlistUserCreated || []).findIndex(
                                        (song) => song.id == libraryInfo.id,
                                    ) == -1
                                "
                                @click="librarySub(libraryInfo.id)"
                            >
                                <svg
            ...
            <div class="introduce-other">
                <div class="introduce-1" v-show="!isSinger">
                    {{ isAlbum ? "发行时间" : "创建时间" }} {{ createTime }}
                </div>
                <div class="introduce-1" v-if="libraryInfo.follow">
                    {{ libraryInfo.follow.fansCnt }} 已关注
                </div>
                <div
                    class="introduce-2"
                    @click="introduceDetailShow = !introduceDetailShow"
                >
                    查看详情
                </div>
            </div>
            ...
            >
                <div
                    ...
                    :class="{
                        'introduce-detail-text-active':
                            introduceDetailShowDelay,
                    }"
                    v-if="introduceDetailShow"
                >
                    <div class="detail-text">
                        <p class="text">
                            {{
                                libraryInfo.description ||
                                libraryInfo.briefDesc ||
                                "暂无描述"
                            }}
                        </p>
                    </div>
                    <div
                        class="text-close"
                        @click="introduceDetailShow = false"
                    >
                        <svg
                            t="1671966797621"
                            class="icon"
        ...
        <div class="library-option">
            <div class="library-type" v-if="isSinger">
                <span
                    :class="{ 'type-selected': artistPageType == 0 }"
                    @click="changeType(0)"
                    >热门单曲</span
                >
                <span
                    :class="{ 'type-selected': artistPageType == 1 }"
                    @click="changeType(1)"
                    >专辑</span
                >
                <span
                    :class="{ 'type-selected': artistPageType == 2 }"
                    @click="changeType(2)"
                    >MV</span
                >
            </div>
            <div
                class="library-playall"
                v-show="isSongList || (isSinger && artistPageType == 0)"
            >
                <div class="playall">
                    <svg
                        t="1668421583939"
                    ...
                    <span @click="playAll('other', librarySongs)"
                        >播放全部</span
                    >
                </div>
                <div class="playall-line"></div>
                <span @click="playAll('other', librarySongs)" class="playall-en"
                    >PLAYALL</span
                >
            </div>
        </div>
        ...
        <LibrarySongList
            ...
            :class="{ 'library-content2': artistPageType == 0 && isSinger }"
            v-show="artistPageType == 0"
        ></LibrarySongList>
        <LibraryAlbumList
            :albumlist="libraryAlbum"
            class="library-content3"
            v-show="artistPageType == 1"
        ></LibraryAlbumList>
        <LibraryMVList
            :mvlist="libraryMV"
            class="library-content3"
            v-show="artistPageType == 2"
        ></LibraryMVList>
    </div>
</template>
