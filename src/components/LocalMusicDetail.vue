...
const {
    currentType,
    currentSelectedInfo,
    currentSelectedSongs,
    currentSelectedFilePicUrl,
    ...
    <div class="local-music-detail">
        <div class="local-music-container">
            ...
            <div class="local-music-hearder">
                <div
                    class="local-music-cover"
                    :class="{
                        'cover-shadow':
                            currentSelectedFilePicUrl != null &&
                            currentType != 'localFiles',
                    }"
                >
                    <img
                        :src="currentSelectedFilePicUrl"
                        v-show="
                            currentSelectedFilePicUrl != null &&
                            currentType != 'localFiles'
                        "
      ...
      > -->
                <div
                    class="list-item"
                    :class="{ 'list-item-playing': songId == item.id }"
                    @dblclick="play(item.id, index)"
                    @contextmenu="openMenu($event, item)"
                    v-for="(item, index) in currentSelectedSongs"
                >
                    <div class="item-title">
                        <div class="item-state">
                            ...
                            <div class="item-num" v-show="!(songId == item.id)">
                                {{ index + 1 }}
                            </div>
                        </div>
                        <div class="item-info">
                            <span class="item-name">{{
                                item.common.title || item.common.localTitle
                            }}</span>
                            <div class="item-format">
                                ...
                                <span class="format"
                                    >{{ item.format.sampleRate / 1000 }}KHz/{{
                                        item.format.bitsPerSample
                                    }}Bits/{{
                                        Math.round(item.format.bitrate / 1000)
                                    }}Kpbs</span
                                >
                            </div>
                        </div>
                    ...
                    <div class="item-other">
                        <div class="item-author">
                            <span
                                ...
                                v-if="
                                    item.common.artists &&
                                    item.common.artists[0] != '其他'
                                "
                                v-for="(singer, index) in item.common.artists"
                                >{{ singer
                                }}{{
                                    index == item.common.artists.length - 1
                                        ? ""
                                        : "/"
                                }}</span
                            >
                        ...
                        <span class="item-time">{{
                            songTime2(item.format.duration)
                        }}</span>
                    </div>
                </div>
                <!-- </RecycleScroller> -->
