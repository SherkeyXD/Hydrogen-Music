...
import { pauseMusic } from "@/utils/player.mjs";
import { addSong, setShuffledList } from "@/utils/player.mjs";
...
const getPositon = () => {
    document
        .getElementsByClassName("playlist-widget-item")[0]
        .scrollTo({ top: currentIndex.value * 37, behavior: "smooth" });
    document
        .getElementsByClassName("playlist-widget-item")[1]
        .scrollTo({ top: currentIndex.value * 37, behavior: "smooth" });
};
</script>
    ...
    <div class="playlist-widget">
        <div class="playlist-widget-header">
            ...
            <div>
                <svg
                    t="1676113510483"
                    @click="getPositon()"
                    class="playlist-widget-position"
                ...
                >
                    <path
                        d="M224.561403 320 224.561403 896 778.105263 896 778.105263 320 712.982456 320 712.982456 832 289.68421 832 289.68421 320 224.561403 320 224.561403 320ZM387.368421 320.664062 452.491228 320.664062 452.491228 768.664062 387.368421 768.664062 387.368421 320.664062 387.368421 320.664062ZM550.175438 320.664062 615.298246 320.664062 615.298246 768.664062 550.175438 768.664062 550.175438 320.664062 550.175438 320.664062ZM387.368421 192 192 192 192 256 810.666664 256 810.666664 192 647.859649 192 647.859649 128 387.368421 128 387.368421 192 387.368421 192Z"
                        p-id="4515"
                    ></path>
                </svg>
            </div>
        ...
        >
            <div
                class="list-item"
                :class="{
                    'list-item-playing': songId == item.id,
                    'list-item-disabled':
                        item.playable !== undefined && !item.playable,
                }"
            ...
            >
                <div class="item-info">
                    ...
                    <span
                        ...
                        >{{ singer.name
                        }}{{ index == item.ar.length - 1 ? "" : "/" }}</span
                    >
                </div>
                <svg
                    t="1670569532229"
                    @dblclick.stop
                    @click="delCurrentSong(index, item.id)"
                    class="item-delete"
                    viewBox="0 0 1024 1024"
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
...
.playlist-widget {
    ...
    transition: 0.3s cubic-bezier(0.19, 0.8, 0.49, 0.99);
    overflow: hidden;
    display: flex;
