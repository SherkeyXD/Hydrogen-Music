...
export const useLibraryStore = defineStore("libraryStore", {
    ...
    actions: {
        ...
        updateUserPlaylist(playlist) {
            this.playlistUserCreated = playlist.splice(
                0,
                this.playlistCount.createdPlaylistCount,
            );
            this.playlistUserSub = playlist.splice(
                0,
                this.playlistCount.subPlaylistCount,
            );
        },
        ...
        async updatePlaylistDetail(id) {
            ...
            await Promise.all([
                ...
                this.librarySongs = mapSongsPlayableStatus(
                    results[1].songs,
                    results[1].privileges,
                );
                if (results[0].playlist.trackIds.length > 1000) {
                    for (
                        let i = 1;
                        i < results[0].playlist.trackIds.length / 1000;
                        i++
                    ) {
                        const params = {
                            id: id,
                        ...
                        const songs = mapSongsPlayableStatus(
                            res.songs,
                            res.privileges,
                        );
        ...
        async updateAlbumDetail(id) {
            ...
            await Promise.all([getAlbumDetail(params), albumDynamic(id)]).then(
                (results) => {
