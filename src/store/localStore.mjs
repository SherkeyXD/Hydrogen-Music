    ...
    actions: {
        ...
        removedup(arr, batch) {
            ...
            let uniqueArr = arr.reduce(function (total, item) {
                obj[item[batch]]
                    ? ""
                    : (obj[item[batch]] = true && total.push(item));
                return total;
            }, []);
            return uniqueArr;
        },
        ...
        updateLocalMusicDetail(type, query, id) {
            ...
            if (type == "localAlbum") {
                const index = (this.localMusicClassify.albums || []).findIndex(
                    (item) => item.id == id,
                );
                this.currentSelectedInfo = {
                ...
                this.currentSelectedSongs =
                    this.localMusicClassify.albums[index].songs;
                if (this.currentSelectedSongs)
                    this.getImgBase64(
                        this.currentSelectedSongs[0].common.fileUrl,
                    ).then((res) => {
                        this.currentSelectedFilePicUrl = res;
                    });
            }
            if (type == "localArtist") {
                const index = (this.localMusicClassify.artists || []).findIndex(
                    (item) => item.id == id,
                );
                this.currentSelectedInfo = {
                ...
                this.currentSelectedSongs =
                    this.localMusicClassify.artists[index].songs;
                if (this.currentSelectedSongs)
                    this.getImgBase64(
                        this.currentSelectedSongs[0].common.fileUrl,
                    ).then((res) => {
